...
def logs_dir(model: str) -> Path:
    """
    Returns the directory where HTCondor logs are stored.
    """
    return Path(htcondor_dir()) / "logs" / model / "prescan"

def submissions_dir(model: str) -> Path:
    """
    Returns the directory where HTCondor submission files are stored.
    """
    return Path(htcondor_dir()) / "submissions" / model / "prescan"

def render_template(template_file: Path, context: dict) -> str:
    with open(template_file) as f:
        return Template(f.read()).render(context)

def make_dirs(model: str) -> None:
    (logs_dir(model) / "out").mkdir(parents=True, exist_ok=True)
    (logs_dir(model) / "log").mkdir(parents=True, exist_ok=True)
    (logs_dir(model) / "err").mkdir(parents=True, exist_ok=True)
    submissions_dir(model).mkdir(parents=True, exist_ok=True)
...
def scan_htcondor(model: str,
    ...
    job_name = f"prescan_{model}_X{int(xmass)}_S{int(smass)}"

    make_dirs(model)

    # Paths
    job_script_path = submissions_dir(model) / f"{job_name}.sh"
    submit_file_path = submissions_dir(model) / f"{job_name}.sub"

    # Render shell script
    job_script = render_template(TEMPLATES_DIR / "prescan_template.sh.j2", {"xmass": xmass,
                                                                            "smass": smass,
                                                                            "model": model,
                                                                            "npoints": num_points})
