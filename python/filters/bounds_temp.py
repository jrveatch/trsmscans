...
def process_row(row,
    ...
    # Extract masses and properties - cast as float explicitly
    if HName != "H2":
        RH, RS, RX = float(row['R11']), float(row['R21']), float(row['R31'])
    else:
        RH, RS, RX = float(row['R21']), float(row['R11']), float(row['R31'])
    mH, mS, mX = float(row[f'm{HName}']), float(row[f'm{SName}']), float(row[f'm{XName}'])
    w_H, w_S, w_X = float(row[f'w_{HName}']), float(row[f'w_{SName}']), float(row[f'w_{XName}'])
    ...
    # Set particle widths again to avoid issues with BR > 1
    H.setTotalWidth(w_H)
    S.setTotalWidth(w_S)
    X.setTotalWidth(w_X)
    ...
    return int(bounds_result.allowed), int(HS_allowed)

def filter_bounds_parallel(dataframe: pd.DataFrame,
                           header_bounds: str,
                           header_signals: str,
                           model_name: str,
                           masses: Masses
    ...
    """
    Optimized filter_bounds function with parallel processing.
    """
    #pred = get_higgs_predictions(model_name=model_name)
    #bounds = get_higgs_bounds()
    #signals = get_higgs_signals()
    ...
    with Pool() as pool:
        results = pool.starmap(
            process_row,
            [(row, model_name, masses) for row in rows]
        )
    ...
    return
    #return pd.Series(filt_bounds, index=dataframe.index), pd.Series(filt_signals, index=dataframe.index)
