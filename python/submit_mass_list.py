...
def submit_mass_list(model: str,
                     decay: str,
                     identifier: str) -> None:
    """
    Submits HTCondor jobs for a list of mass points based on permutations of two values.
    ...
    for XMass, SMass, resolvable in permutations:
        ...
        result = subprocess.run(["python", "submit_single_scan_point.py", *arg_list], text=True)
        print(f"{result}\n")
...
if __name__ == "__main__":
    ...
    submit_mass_list(model=args.model,
                     decay=args.decay,
