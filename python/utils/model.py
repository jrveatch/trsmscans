...
class Model:
    ...
    Attributes:
        ...
        masses (Dict[str, float]): Mapping from scalar particle names to their masses.
        particles (Dict[str, Any]): Particle groupings (e.g., 'SMHiggs', BSM scalars).
        input_parameters (Dict[str, Any]): Input parameter metadata.
        output_parameters (Dict[str, Any]): Output parameter metadata.
        width_parameters (Dict[str, Dict[str, str]]): Width parameter metadata (derived).
        SMHiggs (str): Name of the SM-like Higgs particle.
        BSMScalars (List[str]): List of BSM scalar particle names.
        AllScalars (List[str]): Complete list of scalar particles.

    Properties:
        model_dir (str): Directory path where model files are stored.
        yaml_name (str): Path to the YAML config file.
        template_ini (str): Path to the model's .ini template file.
        mass_string (str): Mass tag like 'X400_S150'.
        input_parameter_names (Tuple[str, ...]): Names of input parameters.
        output_parameter_names (Tuple[str, ...]): Names of output parameters.
        width_parameter_names (Tuple[str, ...]): Names of width parameters.
        all_parameter_names (Tuple[str, ...]): Combined list of all parameter names.
        input_parameter_full_names (Tuple[str, ...]): Full names of input parameters.
        ini_name_to_fullname_map (Dict[str, str]): Maps ini-style parameter names to full names.
        fullname_to_ini_name_map (Dict[str, str]): Maps full names to ini-style parameter names.

    Methods:
        get_mass(name: str) -> float:
            Returns the mass of a scalar particle by original or H_i name.

        get_ordered_scalar_name(particle_name: str) -> str:
            Returns the H_i name for a given scalar particle.

        input_parameter(par_name: str) -> Dict[str, Any]:
            Returns metadata for a given input parameter.

        starting_min(par_name: str) -> float:
            Returns the default minimum value for a given parameter.

        starting_max(par_name: str) -> float:
            Returns the default maximum value for a given parameter.
    """
