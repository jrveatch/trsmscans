...
class ParamSpace:
    ...
    Attributes:
        model (Model): The model object providing particle and parameter definitions.
        decay (str): The decay mode associated with this scan.
        name (str): Identifier for the parameter space instance.

    Properties:
        parameter_names (Tuple[str, ...]): Names of all input parameters.
        parameter_ranges (Dict[str, ParamRange]): Mapping from names to parameter ranges.
        model_name (str): Name of the associated model.
        mass_string (str): Encoded string representing scalar masses (e.g., 'X400_S150').
        mH1 (float): Mass of lightest scalar (H1).
        mH2 (float): Mass of second-lightest scalar (H2).
        mH3 (float): Mass of heaviest scalar (H3).

    Methods:
        center_point() -> Point:
            Returns a point at the center of all parameter ranges.

        random_point() -> Point:
            Samples a point uniformly from the parameter space.

        scale_ranges(new_point: Optional[Point], range_scale: float):
            Re-scales each parameter's range around a center point or the current center.

        reposition_center(point: Point):
            Updates each parameter's range to center around the values in the given Point.

        update_low_high(low_dict: Optional[Dict[str, float]], high_dict: Optional[Dict[str, float]]):
            Updates low/high bounds using provided dictionaries.

        ranges() -> Tuple[Tuple[float, float], ...]:
            Returns a tuple of (low, high) bounds for each parameter.

        widths() -> Tuple[float, ...]:
            Returns a tuple of the widths of each parameter's range.

        volume() -> float:
            Returns the volume (product of widths) of the parameter space.

        write_ini(ini_name: str):
            Fills a model template .ini file with parameter bounds and writes it to disk.

        split_range(param_name: str, split_values: List[float]) -> List[ParamSpace]:
            Splits one parameter's range into multiple ParamSpace objects based on split values.

        log_bounds_table():
            Logs a summary table of min/max bounds for all parameters.

        log_range_table():
            Logs a summary table of current scan ranges (low/high) for all parameters.

        __getitem__(key: str) -> ParamRange:
            Accesses the range object for a given parameter.

        __iter__():
            Enables iteration over ParamRange objects.

        __str__() -> str:
            Returns a formatted string describing the parameter space.
    """
